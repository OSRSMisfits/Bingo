...
<template>
    ...
    <div :class="{ 'rules-overlay': true, show: renderRules, background: !showRules }" @click="closeRules">
        <ol class="rules-list">
            ...
            <li class="rule">There will be a Secret Code released when the event starts that must be in all screenshots for them to count. There is a plugin for Wise Old Man that I would highly recommend installing if you have not already that will keep the Secret Code on the screen along with the date and time. This is not needed but very nice to have so you don’t have to remember to input the Code every time you get a drop. If the code is not in the screenshot. The screenshot will not count towards updating your board and continuing to the next tile. IF AT ANYTIME THE CODEWORD IS NOT IN THE PHOTO THE TEAM WILL GET MOVED BACK TO THAT TILE.</li>
            <li class="rule">If your team rolls a tile and goes dry on completing the tile for 24 hours. You have the option to get 50 Last Man Standing points between your members. You CAN NOT start LMS until the 24 hour time frame has been met. (before and after screenshots of points for each person contributing needed) Doing this will not give you a new dice roll, but instead you will be allowed to choose to move ahead one tile or back one tile. This rule can only be used 3 times per team for the entire event.</li>
        </ol>
  </div>
</template>
