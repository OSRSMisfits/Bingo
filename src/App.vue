...
<template>
  ...
  <div :class="{ 'rules-overlay': true, show: renderRules, background: !showRules }" @click="closeRules">
    <ul class="rules-list">
      ...
      <li class="rule">In previous bingos we have had it so if your team pulls a drop with a different team. Both teams receive full credit. This time the team who pulls the drop receives full credit and the other team there will still get the tile complete but will receive 5 less points. The team with the lesser points with have the bingo tile marked with a red logo instead of purple. This team can go pull an item in their name to receive the other 5 points if they wish to do so. For example Team A and Team B do the Theatre of Blood. Team A pulls the purple and receives full points. Team B for being there will receive 5 less points and have a red logo on the bingo board for that tile. Team B's members can continue sending the raid and pulling their own purple for the 5 points they werent awarded.</li>
    </ul>
  </div>
